...
from EmotionModel.EmotionClassifierModel import EmotionClassifierBERT
from EmotionModel.EmotionClassifierDataset import EmotionClassifierDataset
import numpy as np
np.bool = np.bool_
...
class EmotionClassifierPredict:
    def __init__(self, weight_path, device):
        print("[INFO] LOADING <Emotion-Classifier-Model> ...")

        self.device = device
        self.weight_path = weight_path
        print(f"\t[INFO] DEVICE: {self.device}")

        print("\t[INFO] GET <KoBERT-Tokenizer> ...")
        ...
        self.model = EmotionClassifierBERT(self.bertmodel, dr_rate=0.5).to(self.device)

        print("\t[INFO] APPLYING <emotion-classifier weights> ...")
        self.model.load_state_dict(torch.load(self.weight_path, map_location=self.device))
        self.model.eval()

        print("[DONE] LOAD <Emotion-Classifier-Model> SUCCESS")
